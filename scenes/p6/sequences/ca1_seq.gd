...
@onready var ca1_anim: AnimationPlayer = %CosmoArrow1Anim
...
func move_exalines():
	...
	if inner_first:
		for key in party:
			party[key].move_to(P6Pos.INNER_EXALINE_DODGE_POS[0])
		await get_tree().create_timer(4.0).timeout
		for key in party:
			party[key].move_to(P6Pos.INNER_EXALINE_DODGE_POS[1])
		await get_tree().create_timer(4.0).timeout
		for key in party:
			party[key].move_to(P6Pos.INNER_EXALINE_DODGE_POS[2])
		await get_tree().create_timer(6.0).timeout
		for key in party:
			party[key].move_to(P6Pos.INNER_EXALINE_DODGE_POS[3])
		await get_tree().create_timer(0.5).timeout
		for key in party:
			party[key].move_to(P6Pos.COSMO_DIVE_POS[key])
	else:
		for key in party:
			party[key].move_to(P6Pos.OUTER_EXALINE_DODGE_POS[0])
		await get_tree().create_timer(4.0).timeout
		for key in party:
			party[key].move_to(P6Pos.OUTER_EXALINE_DODGE_POS[1])
		await get_tree().create_timer(4.0).timeout
		for key in party:
			party[key].move_to(P6Pos.OUTER_EXALINE_DODGE_POS[2])
		await get_tree().create_timer(2.0).timeout
		for key in party:
			party[key].move_to(P6Pos.OUTER_EXALINE_DODGE_POS[3])
		await get_tree().create_timer(2.0).timeout
		for key in party:
			party[key].move_to(P6Pos.OUTER_EXALINE_DODGE_POS[4])
		await get_tree().create_timer(0.5).timeout
		for key in party:
			party[key].move_to(P6Pos.COSMO_DIVE_POS[key])
...
func move_auto_attacks():
	for key in party:
		party[key].move_to(P6Pos.COSMO_ARROW_1_AUTO_POS[key])
