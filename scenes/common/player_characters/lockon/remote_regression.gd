...
class_name RemoteRegression

signal remote_regression_stretched(chain_target: Node3D, chain_source: Node3D, stacks: int, chain: RemoteRegression)
...
var color1: Color = Color.MEDIUM_BLUE
var color2: Color = Color.LIGHT_GRAY
...
func _ready():
	...
	material.set_shader_parameter("alpha", 0.8)
...
func _physics_process(_delta: float) -> void :
	...
	if dist_to_target >= max_length:
		remote_regression_stretched.emit(target, source, 2, self)
		return # remove remote_regression debuff, apply two stacks of magic vuln
