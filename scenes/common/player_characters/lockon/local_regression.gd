...
class_name LocalRegression

signal local_regression_squeezed(chain_target: Node3D, chain_source: Node3D, stacks: int, chain: RemoteRegression)
...
var color1: Color = Color.DARK_RED
var color2: Color = Color.DARK_GREEN
...
func _ready():
	...
	material.set_shader_parameter("alpha", 0.75)
...
func _physics_process(_delta: float) -> void :
	...
	if dist_to_target <= min_length:
		local_regression_squeezed.emit(target, source, 2, self)
